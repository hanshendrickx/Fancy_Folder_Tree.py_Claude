...
class FileTypeIcons:
    ...
    # Artisanal icon set (more detailed)
    ARTISANAL = {
        # Documents
        '.txt': '📄', '.md': '📝', '.pdf': '📕', '.doc': '📘', '.docx': '📘',
        '.rtf': '📄', '.odt': '📄', '.pages': '📄',
        ...
        # Config
        '.json': '⚙️', '.xml': '⚙️', '.yml': '⚙️', '.yaml': '⚙️', '.ini': '⚙️',
        '.conf': '⚙️', '.config': '⚙️', '.toml': '⚙️',
        
        # Executables
        '.exe': '⚙️', '.msi': '⚙️', '.deb': '📦', '.rpm': '📦', '.dmg': '📦',
        '.app': '📱', '.apk': '📱',
    ...
    # Professional icon set (clean symbols)
    PROFESSIONAL = {
        '.txt': '[T]', '.md': '[M]', '.pdf': '[P]', '.doc': '[D]', '.docx': '[D]',
        '.xls': '[X]', '.xlsx': '[X]', '.csv': '[C]',
        '.jpg': '[I]', '.jpeg': '[I]', '.png': '[I]', '.gif': '[I]',
        '.mp3': '[♪]', '.wav': '[♪]', '.mp4': '[V]', '.avi': '[V]',
        '.py': '[Py]', '.js': '[JS]', '.html': '[H]', '.css': '[S]',
        '.zip': '[Z]', '.rar': '[R]',
        'folder': '[+]', 'file': '[ ]', 'unknown': '[?]'
    }
...
class FolderTreeGenerator:
    ...
    def generate_tree(self, root_path: str, show_size: bool = False, 
        ...
        # Add root directory
        root_icon = self.get_file_icon(root)
    ...
    def _build_tree(self, directory: Path, prefix: str, depth: int, show_size: bool,
        ...
        for i, entry in enumerate(entries):
            ...
            # Build tree symbols
            if is_last:
                current_prefix = prefix + self.style['corner'] + self.style['horizontal']
                next_prefix = prefix + self.style['space'] + self.style['space']
            else:
                current_prefix = prefix + self.style['junction'] + self.style['horizontal']
                next_prefix = prefix + self.style['vertical'] + self.style['space']
    ...
    def save_text(self, output_path: str, header: bool = True) -> None:
